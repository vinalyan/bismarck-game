...
const Hex: React.FC<HexProps> = ({
  ...
  const getHexStyle = () => {
    ...
    // Подсветка доступных гексов для движения и заправки
    // Приоритет: заправка > движение > поиск
    if (isRefuelAvailable && isAvailableForMovement) {
      // Комбинированный стиль: гекс доступен и для движения, и для заправки
      // Показываем фиолетовый (заправка важнее, так как это специальное действие)
      stroke = '#8e44ad'; // Фиолетовый для гексов заправки
      strokeWidth = 3; // Более толстая обводка для комбинированного
      ...
      // Подсветка доступных гексов для движения (зеленый)

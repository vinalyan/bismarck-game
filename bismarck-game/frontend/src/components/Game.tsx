...
const Game: React.FC = () => {
  ...
  // useEffect для loadCurrentTurn удален
    ...
    const handleGameEventReceived = async (event: CustomEvent) => {
      ...
      switch (eventData.event) {
        ...
        case 'turn_completed':
          ...
          // Удален вызов phaseAPI.getCurrentPhase - информация о текущей фазе теперь приходит через GameModel
          // TODO: Извлечь информацию о текущей фазе из GameModel и обновить setCurrentTurn
          break;
      ...
      if (response.success) {
        ...
        try {
          ...
          if (updatedUnits.success && updatedUnits.data) {
            ...
            if (playerSideForSearch !== 'unknown') {
              const updatedData = updateGameDataFromModel(
                updatedUnits,
                currentGame,
                playerSideForSearch,
      ...
      <div className="game-content">
        ...
        >
          <HexMap
            ...
            authToken={authToken}
            onRefreshData={async () => {
              // Используем централизованную функцию обновления данных
              await handleGameModelUpdate();
            }}

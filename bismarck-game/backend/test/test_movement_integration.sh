...
create_test_game() {
    ...
    local game_data='{"name":"Integration Test Game","description":"Test game for integration testing","side":"german"}'
...
if [ -n "$GAME2_ID" ]; then
    ...
    # Подключение первого игрока ко второй игре
    JOIN_RESULT2=$(join_game "$TOKEN1" "$GAME2_ID" "german")
    if [ "$JOIN_RESULT2" = "true" ]; then
        echo -e "${GREEN}✅ Первый игрок подключен ко второй игре${NC}"
    else
        echo -e "${RED}❌ Не удалось подключить первого игрока ко второй игре${NC}"
    fi

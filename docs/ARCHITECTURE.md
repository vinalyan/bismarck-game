...
**Примечание:** Точка входа сервера находится в отдельном файле (см. Makefile: `go build -o bin/server ./cmd/server`). Структура `cmd/server/` может быть создана при необходимости.

### Ключевые компоненты

#### 1. Server (точка входа)
...
- `AirAttackService` - управление маркерами воздушной атаки
...
type GameModel struct {
    ...
    AirAttack   *AirAttackData              // Маркеры воздушной атаки
...
**AirAttackData** - структура для хранения маркеров воздушной атаки:
- `German` - карта гексов с количеством маркеров для немецкой стороны (hexID -> количество)
- `Allied` - карта гексов с количеством маркеров для союзников (hexID -> количество)
- Маркеры хранятся в GameModel и автоматически очищаются в фазе администрирования
...
**Интеграция с AirAttackService:**
- AirAttackPhaseHandler проверяет наличие валидных маркеров (маркеры в гексах с вражескими кораблями)
- Если валидных маркеров нет, фаза автоматически завершается
- AdminPhaseHandler очищает все маркеры воздушной атаки при старте фазы администрирования
...
**AirAttackService** - управление маркерами воздушной атаки
- Добавление/удаление маркеров воздушной атаки в гексы
- Получение списка маркеров для стороны игрока
- Получение целей для воздушной атаки в гексе (вражеские корабли)
- Хранение маркеров в GameModel.AirAttack (отдельно для German и Allied)
- Интеграция с GameStateService для обновления GameModel
- Интеграция с GameEventService для логирования событий
- Автоматическая очистка маркеров в фазе администрирования
...
- `AirAttackHandler` - управление маркерами воздушной атаки
...
│   │   ├── AirAttackPanel.tsx # Панель воздушной атаки
│   │   ├── AirAttackModal.tsx # Модальное окно выбора целей
...
│   └── utils/            # Утилиты
├── editor/               # Редактор структур карты
...
- `phaseAPI.ts` - фазы
- `searchAPI.ts` - поиск
- `airAttackAPI.ts` - управление маркерами воздушной атаки
- `refuelAPI.ts` - заправка кораблей
- `shipsAPI.ts` - конфигурация кораблей
- `mapService.ts` - структуры карты

**Особенности API клиентов:**
- Автоматическое добавление JWT токена в заголовки через axios interceptors
- Обработка 401 ошибок с автоматическим перенаправлением на экран логина через GameStore (без использования `window.location.href` для корректной работы в SPA)
- Защита от множественных одновременных запросов в компонентах (использование `useRef` для отслеживания состояния загрузки)
...
**AirAttackPanel.tsx** - панель воздушной атаки:
- Отображение активных маркеров воздушной атаки
- Инициирование атаки по выбранному гексу
- Фильтрация маркеров (только в гексах с вражескими кораблями)
- Интеграция с AirAttackModal для выбора целей

**AirAttackModal.tsx** - модальное окно выбора целей:
- Отображение доступных целей в гексе
- Выбор типа корабля для атаки
- Отправка команды на выполнение атаки

**Editor** (`frontend/editor/`) - редактор структур карты:
- Визуальный редактор для создания структур карты
- Создание портов, каналов, маршрутов конвоев
- Разметка воздушных секторов и зон
- Экспорт/импорт структур в JSON формате
...
1. **GameModel как единый источник истины:**
   ...
   - GameModel содержит все данные о юнитах, Task Forces, событиях, поиске и т.д.
...
# Запуск сервера
make run
# Или напрямую: go run cmd/server/main.go (если файл существует)
```
...
Редактор запускается на порту 3001 и позволяет создавать и редактировать структуры карты (порты, каналы, маршруты конвоев и т.д.).
...
- Правила игры: `docs/Правила.md`
- Правила для ИИ агента: `docs/AI_AGENT_RULES.md`
